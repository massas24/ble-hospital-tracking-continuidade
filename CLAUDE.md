...
**Acquisition config is remote, not compiled-in (guião secção 7)**: `scan_duration_sec`/`upload_interval_ms` are fetched once at boot from `GET /api/node-config?esp_id=...` (unauthenticated, same precedent as `/api/bledata`) — optional per-`esp_id` override stored on `esp_mapping` (independent of `room`; a mapping doc can have either, both, or neither), falling back to `DEFAULT_SCAN_DURATION_SEC`/`DEFAULT_UPLOAD_INTERVAL_MS` when unset. A node that can't reach the endpoint at boot falls back to those same defaults rather than failing to start. `POST /api/esp-mapping` (config change) and `DELETE /api/delete-room/<esp_id>` (mapping removal) are both blocked (409) while `current_experiment_id` is active, since either would corrupt data collected mid-trial. The effective per-batch value is stamped onto every `raw_detections` row regardless; `analyze_room_decisions.py`'s `compute_acquisition_config_divergence` cross-checks it offline against what was registered in `experiments`, but only compares a single registered value when every `esp_id` in that trial agrees with the others — a genuinely heterogeneous per-node config (the reason the override is scoped per-`esp_id` at all) must never be flagged as an error just because it differs from a single registered number.
...
**Auth is intentionally minimal**: `@auth_required` only checks that an `X-User` header is non-empty — it doesn't validate the user exists or check a password. Every `/api/*` route uses it except `/api/bledata`, `/api/node-config`, `/api/signup`, `/api/login`, `/api/forgot-password`, `/api/reset-password`. Don't assume stronger access control exists than this.
...
**Firmware** (`esp32-firmware/rtls_node/rtls_node.ino`): syncs time via NTP at boot and periodically, fetches its acquisition config once at boot (see above; falls back to compiled-in defaults on failure), sends the batch payload shape ingestion expects (`esp_id`, sequential `node_seq`, per-boot `boot_id`, optional `node_time`, effective `scan_duration_sec`/`upload_interval_ms`). `NODE_CONFIG_URL` is derived from `SERVER_URL` at runtime (path substitution), never a second hardcoded constant — there is exactly one IP/port to edit in the file. No store-and-forward, retry/backoff, or node-level auth exists yet on either the firmware or backend side of `/api/bledata`.
